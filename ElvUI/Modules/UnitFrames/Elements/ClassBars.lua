...
function UF:UpdateClassBar(current, maxBars, hasMaxChanged, poverType)
	...
	if not frame.USE_CLASSBAR or current == 0 or maxBars == nil then
		self:Hide()
		if isShown then
			stateChanged = true
		end
...
function UF:Construct_DeathKnightResourceBar(frame)
	...
	runes.PostUpdateVisibility = PostUpdateRunes
