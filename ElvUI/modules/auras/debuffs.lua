...
timerGroup:SetScript('OnFinished', function(self, requested)
	...
	for i, button in ipairs(buttons) do
		...
		if(debuff) then
			...
			local statusbar = A.db.debuffs.statusbar;
			local noduration = A.db.debuffs.noduration;
			local enableThreshold = A.db.debuffs.tenable;
			local showText = A.db.debuffs.showText;
