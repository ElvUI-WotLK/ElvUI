...
timerGroup:SetScript('OnFinished', function(self, requested)
	...
	for i, button in ipairs(buttons) do
		...
		if(debuff) then
			...
			local timeBar = A.db.debuffs.timeBar.enable;
			local noDuration = A.db.debuffs.timeBar.noDuration;
			local enableThreshold = A.db.debuffs.time.tenable;
			local showText = A.db.debuffs.time.showText;
