...
local function OnEnter(self)
	...
	if numBNetOnline > 0 then
		DT.tooltip:AddLine(' ')
		DT.tooltip:AddLine(battleNetString)
		...
		for i = 1, #BNTable do
			...
			if info[7] then
				if info[6] == wowString then
					...
					DT.tooltip:AddDoubleLine(format(clientLevelNameString, info[6],levelc.r*255,levelc.g*255,levelc.b*255,info[16],classc.r*255,classc.g*255,classc.b*255,info[4],groupedTable[grouped], 255, 0, 0, statusTable[status]),info[2].." "..info[3],238,238,238,238,238,238)
					...
					DT.tooltip:AddDoubleLine(format(otherGameInfoString, info[6], info[4]), format(otherGameInfoString2, info[2], info[3]), .9, .9, .9, .9, .9, .9)
				end
			end
		end
	end
...
DT:RegisterDatatext(L['Friends'], {'PLAYER_ENTERING_WORLD', "BN_FRIEND_ACCOUNT_ONLINE", "BN_FRIEND_ACCOUNT_OFFLINE", "BN_FRIEND_INFO_CHANGED", "BN_FRIEND_TOON_ONLINE",
